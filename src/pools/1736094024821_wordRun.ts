...
export function formWordRunFile(wordName: string, template: string): string {
  ...
  Ønconst template = §{template};Ønconst result = §{wordName}(template);Ønconst queue=getQueue();Ønconsole.log(tts(result, false));Ønif(queue.length>0) {Ønconsole.log("|||||||");Ønqueue.forEach(q=>console.log(tts(q,false),"&&&&&&&"))}ØnclearQueue();ェ;
}
...
export const createRunnableGeneratorFileContents = async (
  ...
  const allImports = ェ§{templatesImport}Øn§{generatorsImport}Øn§{wordsImport}ェ;
...
export function formGeneratorFile(
  ...
  const templatesImport = ェimport { §{templates.join(
    ",Øn"
  ...
  const generatorRun = ェ// @ts-ignoreØnconst result = §{generatorString};Ønconsole.log(tts(result,false));ェ;
  return ェ§{templatesImport}Øn§{generatorsImport}Øn§{templateString}Øn§{generatorRun}ェ;
}
`,
'fileContents18': ()=>`
