...
const template = {
'fifth.tsxa4023e8966/elementTag01,elementTag02': ({elementTag01, elementTag02})=>`import { cloneDeep, difference, last, uniqueId } from "lodash";
...
export function useTemplate(
  ...
  let [template, setTemplate] = useState${run(elementTag01, 'elementTag01')}(
    last(definition.wordSteps).result
  );

  const [wordSteps, setWordSteps] = useState${run(elementTag02, 'elementTag02')}(definition.wordSteps);
...
`,
'elementTag01/tagBody01': ({tagBody01})=>`<${run(tagBody01,'tagBody01')}>`,
'elementTag02/tagBody02': ({tagBody02})=>`<${run(tagBody02,'tagBody02')}>`,
'tagBody01': ()=>`Template`,
'tagBody02': ()=>`WordStep[]`
};
// @ts-ignore
